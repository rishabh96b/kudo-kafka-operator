	...
	Describe("[Kafka Sanity Checks]", func() {
		...
		Context("Update LOG_RETENTION_HOURS param from default 168 to 200", func() {
			...
			It("LOG_RETENTION_HOURS should change from 168 to 200", func() {
				...
				err = utils.KClient.WaitForStatus(DefaultKudoKafkaInstance, customNamespace, v1beta1.ExecutionInProgress, 1300)
				Expect(err).To(BeNil())
				err = utils.KClient.WaitForStatus(DefaultKudoKafkaInstance, customNamespace, v1beta1.ExecutionComplete, 1300)
				Expect(err).To(BeNil())
